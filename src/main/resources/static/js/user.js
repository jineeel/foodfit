...
function checkDataId(userId) {
    ...
    userRequest('POST', '/api/user/checkId', userId, success, fail);
}
...
function checkDataPhone(phone) {
    function success(result) {
        if (result) {
            ...
            if(!(phoneOrigin.value===phoneInputEl.value)){
    ...
    userRequest('POST', '/api/user/checkPhone', phone, success, fail);
}
...
function sendTelNumber(phone) {
    ...
    userRequest('POST', '/api/user/sms', JSON.stringify({"to": phone}), success, fail);
}
...
/*
    회원정보 수정시 핸드폰번호
 */
const phoneOrigin = document.getElementById('phoneOrigin');
if(phoneOrigin){
    phoneInputEl.addEventListener('blur',()=>{
        setValidTel(phoneInputEl.value);
    })
}
...
if(emailInputEl) {
    emailInputEl.addEventListener('blur', () => {
        setValidEmail(emailInputEl.value);
    });
}
function setValidEmail(email){
    const emailRegExp = /^[a-zA-Z0-9+-\_.]+@[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+$/;
    if (emailRegExp.test(email)) {
        checkDataEmail(email)
    } else {
...
function checkDataEmail(email) {
    function success(result) {
        if (result) {
            ...
            if(!(emailOrigin.value===emailInputEl.value)) {
    ...
    userRequest('POST', '/api/user/checkEmail', email, success, fail);
}
...
if(updatePwBtn){
    updatePwBtn.addEventListener('click',(e)=>{
        ...
        if(pwInputEl.value&&pw2InputEl.value){
            userRequest('POST', '/api/user/updatePassword', JSON.stringify(formData), success, fail);
        }else{
...
function userRequest(method, url, body, success, fail){
    fetch(url,{
        method: method,
        body: body,
        headers: {
